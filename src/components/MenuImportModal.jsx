...
export default function MenuImportModal({ restaurant, onClose, onImported }) {
  ...
  const handleConfirm = useCallback(async () => {
    ...
    setResult({ categoriesImported, itemsImported, itemsSkipped })
    setWriteErrors(errors)
    setWriting(false)
    setStep(STEP_DONE)
    if (onImported) onImported()
  }, [scraped, restaurant.id, onImported])
    ...
    >
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-5">
          ...
          {step === STEP_PREVIEW && scraped && <PreviewStep scraped={scraped} />}
          {step === STEP_DONE && result && (
            <DoneStep result={result} writeErrors={writeErrors} />
          )}
        </div>
        ...
        <div className="p-5 border-t flex justify-end gap-3">
          ...
          {step === STEP_PREVIEW && (
            <>
              <button
                onClick={() => { setStep(STEP_URL); setScraped(null) }}
                disabled={writing}
...
function DoneStep({ result, writeErrors }) {
