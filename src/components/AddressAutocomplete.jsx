...
function loadMaps() {
  ...
  mapsLoadPromise = import('@googlemaps/js-api-loader').then(({ importLibrary, setOptions }) => {
    setOptions({ key: apiKey })
    return importLibrary('places')
  }).then(() => {
    console.log('[AA] Google Maps Places loaded')
  }).catch(err => {
...
export default function AddressAutocomplete({ defaultValue, onSelect, onChange, className }) {
  ...
  const [loaded, setLoaded] = useState(!!window.google?.maps?.places?.Autocomplete)
